...
import { FileArrowDown, CaretDown } from "@phosphor-icons/react";
...
export function RefundDetail() {
  ...
  const formattedValue = new Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(refund.value / 100);
  ...
  return (
    <main className="flex flex-col w-full max-w-[512px] bg-background rounded-[16px] p-10 mx-auto">
      <div className="flex flex-col mb-10 text-left">
        <h1 className="text-[20px] font-bold text-foreground mb-3 leading-6">Solicitação de reembolso</h1>
      ...
      <div className="flex flex-col gap-6">
        ...
        {/* Option: Abrir comprovante */}
        <div className="flex items-center gap-2 mt-4 ml-2">
          <a
            href={receiptUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-2 text-primary hover:opacity-80 transition-opacity font-semibold mt-1"
          >
            <FileArrowDown size={24} weight="bold" />
            <span className="text-sm">Abrir comprovante</span>
          </a>
        </div>
        ...
        >
          Excluir
        </button>  {/* Wait, the trigger button in Image 2 is just dark bg "Excluir". Updated class to bg-secondary! Actually dark bg is #28362F maybe? No, secondary is dark gray in this theme maybe? */}
      </div>
      ...
      {isDeleteDialogOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div className="bg-background rounded-2xl w-full max-w-[400px] p-8 shadow-xl flex flex-col animate-in fade-in zoom-in duration-200">
            <h2 className="text-xl font-bold text-foreground mb-4">Excluir solicitação</h2>
